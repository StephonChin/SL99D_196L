...
uint16_t  SendCnt;
uint32_t  i2s_buffer1[NUM_I2S_PIXEL_WORDS1];
uint32_t  i2s_send_buffer1[NUM_I2S_PIXEL_WORDS1];
uint32_t  i2s_buffer2[NUM_I2S_PIXEL_WORDS2];
uint32_t  i2s_send_buffer2[NUM_I2S_PIXEL_WORDS2];
sdio_queue_t i2s_pixels_queue1;
sdio_queue_t i2s_pixels_queue2;
bool      UpdateFlag;


#define WS2812_USE_INTERRUPT  1

#if WS2812_USE_INTERRUPT == 1
static ICACHE_RAM_ATTR void ws2812_isr(void)
{
  uint16_t  a;
  volatile uint32_t  status = READ_PERI_REG(SLC_INT_STATUS);
  ...
  if (status & SLC_RX_EOF_INT_ST){
    if(READ_PERI_REG(SLC_RX_EOF_DES_ADDR)==(((uint32_t)&i2s_pixels_queue1))){
      for (a = 0; a < NUM_I2S_PIXEL_WORDS1; a++)    i2s_send_buffer1[a] = i2s_buffer1[a];   
    }
    else{
      for (a = 0; a < NUM_I2S_PIXEL_WORDS2; a++)    i2s_send_buffer2[a] = i2s_buffer2[a];
      UpdateFlag = false;
    }
  }
}
...
void ws2812_dma(sdio_queue_t *i2s_pixels_queue)
{
  ...
  //enable int
  SET_PERI_REG_MASK(I2SINT_ENA,  I2S_I2S_RX_REMPTY_INT_ENA);
#endif
#endif
